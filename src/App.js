...
import { addQuotes, updateLoading, updateError } from './actions';
...
export class App extends Component {
  ...
  componentDidMount() {
    const { currentSeason, isError, updateError } = this.props;
    fetch(`https://the-office-api.herokuapp.com/season/${currentSeason}/format/quotes`)
      .then(res => {
        if (!res.ok) {
          throw Error(isError)
        }
  ...
  setStateBySeason = (season) => {
    ...
    // console.log('currentseason: ', this.props.currentSeason)
    // console.log('loading?',  this.props.isLoading)
    fetch(`https://the-office-api.herokuapp.com/season/${season}/format/quotes`)
    .then(res => {
      if (!res.ok) {
        throw Error(this.props.isError)
      }
      return res.json()
    })
      .then(data => {
        this.setInitialState(data.data);
        // console.log('currentseasoninthen: ', this.props.currentSeason)
      })
  ...
  handleSearch = (query) => {
    ...
    if (query) {
      ...
      let matches = this.props.quotes.reduce((acc, quote) => {
  ...
  render() {
    return (
      <main className="App">
        ...
        { this.props.isLoading && <img src='https://media.giphy.com/media/SwffhF8CK0RTW/giphy.gif' alt='loading....'/> }

        { !this.props.isLoading &&
        <Search
          isSearchDisplay={this.props.searchMatches.length}
...
export const mapDispatchToProps = dispatch => ({
  ...
  updateError: status => dispatch(updateError(status))
})
...
// { (!this.props.isLoading && this.props.searchMatches.length <= 0) ?
//   <DisplayContainer
//     quotes={this.props.quotes}
//     currentEpisode={this.props.currentEpisode}
//   /> :
//   <DisplayContainer
//     quotes={this.props.searchMatches}
//     currentEpisode={this.props.currentEpisode}
//     /> }

// quotes={ this.props.searchMatches.length <= 0 ? this.props.quotes : this.props.searchMatches }
//
